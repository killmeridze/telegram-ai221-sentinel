...
def schedule_text(today: datetime.date, language: str, group: int) -> str:
    ...
    if not schedule:
        message_text = 'Нечего делать, что ли? Лучше иди поспи' if language == 'rus' else 'Нема що робити? Краще іди поспи'
        return message_text

    message_text = f'Расписание на {day_name_ru}:\n\n' if language == 'rus' else f'Розклад на {day_name_uk}:\n\n'
...
def get_quote_tag_from_user(message : types.Message) -> None:
    ...
    bot.register_next_step_handler(msg, change_quote_tag)

def change_quote_tag(message : types.Message) -> None:
    user_language = get_user_language(message.chat.id)
    tag = message.text
